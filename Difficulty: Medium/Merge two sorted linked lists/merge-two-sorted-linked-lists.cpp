...
class Solution {
  public:
    Node* sortedMerge(Node* head1, Node* head2) {
        ...
        // If any nodes are left in head1, append them
        if (head1) {
            current->next = head1;
        }
        // If any nodes are left in head2, append them
        if (head2) {
            current->next = head2;
        }
    
        // Return the merged list (dummy's next is the start of the merged list)
        return dummy->next;
    }
};
